...
import { invalidateRunExportContracts } from "@/lib/dashboardInvalidation";
import {
...
export default function WizardRunner() {
  ...
  const handleSaveAndExport = async () => {
    ...
    const waiverEligible =
      nextEvaluation.waiverEligible ||
      (nextEvaluation.blockers.length > 0 &&
        nextEvaluation.blockers.every((item) => item.waiverAllowed));
    const transitionAction =
      nextEvaluation.status === "pass"
        ? "advance"
        : waiverEligible && waiverReason
        ? "waive"
        : "save_draft";
    const transitionAttempt = createTransitionAttempt({
      ...
      waiverReason: transitionAction === "waive" ? waiverReason : undefined,
    });
    const persistedTransitionAttempt = localDb.createTransitionAttempt(transitionAttempt);

    logAuditEvent({
      eventType:
        transitionAction === "waive"
          ? "stage_waiver_recorded"
          : "stage_transition_recorded",
      actorId: "local_user",
      projectId: plan.context.projectId,
      runId: run.id,
      transitionAttemptId: persistedTransitionAttempt.attemptId,
      details: persistedTransitionAttempt,
    });
    ...
    const { createExportJob, renderPacketPdf } = await loadExportEngineRuntime();
    ...
    const blob = await renderPacketPdf(packet);
    downloadBlob(blob, `${run.id}.pdf`);
  };
        ...
        <div className="grid" style={{ gap: 14 }}>
          ...
          <div className="row wrap">
            ...
            <ActionButton
              ...
              disabled={!plan}
              onMouseEnter={() => prefetchExportRuntime({ includePdf: true })}
              onFocus={() => prefetchExportRuntime({ includePdf: true })}
            >
              Save Run + Export PDF
            </ActionButton>
          </div>
