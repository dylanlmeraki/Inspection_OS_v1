...
describe("exportEngine", () => {
  ...
  it("persists waiver reason into manifest exceptions when waived transitions are linked", async () => {
    const plan = buildWizardPlan(wizardScenarios.contraCostaPrePermit.selection);
    ...
    const persistedTransition = localDb.createTransitionAttempt(transitionAttempt);

    const { manifestRecord } = await createExportJob({
      runId: run.id,
      stageGateEvaluationId: stageGateEvaluation.id,
      transitionAttemptId: persistedTransition.attemptId,
      attachments: [],
    });

    expect(manifestRecord.exceptions[0].waived).toBe(true);
    expect(manifestRecord.exceptions[0].waiverReason).toBe(
      "AHJ confirmed manual bridge approval"
    );
    expect(manifestRecord.exceptions[0].transitionAttemptId).toBe(
      persistedTransition.attemptId
    );
    expect(manifestRecord.waiverSection.action).toBe("waive");
    expect(manifestRecord.waiverSection.waiverReason).toBe(
      "AHJ confirmed manual bridge approval"
    );
    expect(manifestRecord.waiverSection.waivedBlockerCodes).toContain(
      "CONTRA_RECOGNIZED_AGENCY_REQUIRED"
    );
  });
});
